...
import { trackOrderAPI } from "@/services/user.service";
        ...
        <div style={{ display: "grid", gap: "1.15rem" }}>
          <TrackCard
            ...
            onSubmit={handleSubmit(async (values) => {
              const order = await trackOrderAPI(values.orderNumber, values.email);
              if (!order) {
                setResult({
                  ok: false,
                  message:
          ...
          {result?.ok ? (
            <>
              ...
              <TrackCard>
                ...
                <ItemList>
                  {result.order.items.map((item) => (
                    ...
                    >
                      <ItemThumb>
                        <Image
                          src={item.image || (item as any).productImage || "/placeholder.png"}
                          alt={item.name || (item as any).productName || "Product"}
                          fill
                          sizes="48px"
                        />
                      </ItemThumb>
                      <ItemMeta>
                        <strong>{item.name || (item as any).productName}</strong>
                        <span>
                ...
                <Hint>
                  Shipping to {result.order.fullName || (result.order as any).shippingAddress?.fullName},{" "}
                  {result.order.city || (result.order as any).shippingAddress?.city}
                </Hint>
              </TrackCard>
            </>
          ) : null}
